...
interface OrderMenuDao {
    @Query("SELCT * FROM OrderMenu")
    fun getAll(): List<OrderMenu>

    @Query("SELCT om.id as id, om.order_id as orderId, menu.name as menuName, om.count as count from OrderMenu om Left join menu on om.menu_id = menu.id")
    ...
    @Query("SELCT id FROM OrderMenu order by id desc limit 1")
    fun getLastId() : Int
